...
async function syncRemoteGuilds({ silent = false } = {}) {
  ...
  try {
    remoteGuilds = await ipcRenderer.invoke('channels:remoteList');
    renderRemoteGuilds();
...
async function importSelectedGuilds() {
  ...
  try {
    const response = await ipcRenderer.invoke('channels:importRemote', {
      instanceId,
...
window.addEventListener('DOMContentLoaded', () => setTimeout(mountSyncUi, 0));
